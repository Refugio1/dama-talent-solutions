...
foreach ($file in $files) {
    if (Test-Path $file) {
        ...
        $content = [regex]::Replace($content, '(<p class="text-gray-400 text-xs font-light leading-relaxed">.*?</p>\s*)<div class="mt-4">', '$1<div class="mt-auto pt-4">', [System.Text.RegularExpressions.RegexOptions]::Singleline)
        ...
        # 7. Título animado
        $origTitleRegex = '<h1 class="text-4xl md:text-h1 font-heading font-medium tracking-tight text-brand-black leading-tight mb-6">.*?Headhunting Ejecutivo para Sectores.*?<span class="italic font-light text-brand-gray text-rotator-word">.*?</span>.*?en M.*?xico\..*?</h1>'
        $newTitle = "<h1 class=`"text-4xl md:text-h1 font-heading font-medium tracking-tight text-brand-black leading-tight mb-6 text-center`">`n                Headhunting Ejecutivo para Sectores<br>`n                <span class=`"italic font-light text-brand-gray text-rotator-word block my-2`">Técnicos</span>`n                <span class=`"block`">en México.</span>`n            </h1>"
        $content = [regex]::Replace($content, $origTitleRegex, $newTitle, [System.Text.RegularExpressions.RegexOptions]::Singleline)
